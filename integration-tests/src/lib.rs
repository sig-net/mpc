...
pub struct TestContext<'a> {
    pub leader_node: &'a containers::LeaderNodeApi,
    pub pk_set: &'a Vec<Point<Ed25519>>,
    pub worker: &'a Worker<Sandbox>,
    pub signer_nodes: &'a Vec<containers::SignerNodeApi>,
...
pub async fn with_nodes<F>(nodes: usize, f: F) -> anyhow::Result<()>
where
    F: for<'a> FnOnce(TestContext<'a>) -> BoxFuture<'a, anyhow::Result<()>>,
{
    ...
    let relayer_ctx_future = initialize_relayer(&docker_client, NETWORK);
