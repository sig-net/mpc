...
pub async fn signature_production(
    ...
    // let (account, signer) = new_account(ctx).await?;signature_production
    // let (_, payload_hash, tx_hash) = request_sign(ctx, &signer).await?;
    // let signature = wait_for::signature_responded(ctx, tx_hash).await?;
    ...
    let (payload_hashes, tx_hashes): (Vec<_>, Vec<_>) = request_sign_multiple(ctx, amount).await?.into_iter().unzip();
    let signatures = wait_for::signatures_responded(ctx, &tx_hashes).await?;
    // for _ in 0..amount {
    //     // single_signature_production(ctx, state).await?;
    //     let (_, payload_hash, tx_hash) = request_sign(ctx, &signer).await?;
    // }
