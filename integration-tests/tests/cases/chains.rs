...
use mpc_node::sign_respond_tx::hash_rlp_data;
...
async fn test_solana_signature_first_phase_bidirectional() -> anyhow::Result<()> {
    ...
    let payload = [42u8; 32];
    let path = "test/integration/path";
    // Sign-respond requests are currently limited to key version 0 on the node side.
    let key_version = 0;

    let zero_address = AlloyAddress::from_slice(&[0u8; 20]);
    let mock_transaction = EthereumTransaction {
        nonce: U256::from(0u8),
    ...
    let unsigned_rlp = encode_unsigned_legacy_tx(&mock_transaction, 60);
    let unsigned_tx_hash = hash_rlp_data(unsigned_rlp.clone());
    let payload_scalar = k256::Scalar::from_bytes(unsigned_tx_hash)
        .context("unsigned transaction hash exceeds secp256k1 field")?;
    ...
    let s = outcome.signature.s;
    let signature_valid = [0u8, 1u8].into_iter().any(|recovery_id| {
        check_ec_signature(&derived_user_pk, &big_r, &s, payload_scalar, recovery_id).is_ok()
    });
