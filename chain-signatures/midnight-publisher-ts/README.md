...
The publisher and real-stack caller use `@sig-net/midnight`, `@sig-net/midnight-contract`, and `@sig-net/midnight-contract-deploy` version `0.24.0-rc.2`. Request identity commits to `(sender, keyVersion, path, algo, txParamType, txParams, executionDest)`. Reserved signature destination, extra MPC parameters, and serialization schemas remain outside the identity. The EVM account nonce remains part of the transaction.
