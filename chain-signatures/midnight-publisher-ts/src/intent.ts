...
// Both respond circuits take the same signature-only event: the 0.15.0 contract's
// SignatureRespondedEvent and RespondBidirectionalEvent are each a bare Signature.
...
export async function buildIntent(managedDir: string, input: BuildIntentInput): Promise<Uint8Array> {
  ...
  // proves, and the build path performs no proving: only the caller holds the pinned
  // deployment's inputs.
