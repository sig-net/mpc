...
// them in declaration order. `circuit` resolves first regardless, being the discriminator.
const BuildSchema = z.discriminatedUnion(
  "circuit",
  [
    wireObject({
      id: wireId,
    ...
    wireObject({
      ...
      serializedOutput: wireHex(128, MUST_BE_HEX_128),
      outputLen: z.int(MUST_BE_AN_OUTPUT_LEN).min(0, MUST_BE_AN_OUTPUT_LEN).max(128, MUST_BE_AN_OUTPUT_LEN),
      ...pinned,
    }),
  ],
