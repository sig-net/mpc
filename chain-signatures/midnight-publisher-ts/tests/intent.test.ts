// The intent builder, against the real compiled contract and a real captured
// singleton state. Nothing is mocked: if these pass, the circuit ran.
...
import { decodeIntent, fixtureBytes, managedDir, pushedCallArgs, renderCall, toHex } from "./support.js";

// The singleton `respond-singleton-state-37571.mn` was captured from, per
// tests/fixtures/README.md. Its embedded verifier keys match `dist/managed`,
// which is what lets the entry-point lookup succeed.
const SINGLETON = "d7b3c45da613be25050bbdf3fde4cef8f66154d3a52ca8c1edd878bd6391f169";
// Minted by the real circuit on the capture chain; any 32 bytes would do for a
// blind append, but a real one keeps the fixture honest.
...
describe("buildIntent", () => {
  it("carries exactly one respond call for the address it was asked about", async () => {
    const intent = decodeIntent(await buildIntent(managedDir(), respondInput()));

    expect(intent.actions).toHaveLength(1);
    const [action] = intent.actions;
    expect(String(action)).toContain(SINGLETON);
  });
  ...
  it("builds respondBidirectional with its output and length", async () => {
    // `outputLen` is what tells a consumer how much of the fixed Bytes<128> is real
    // return data. Zeroed or dropped, the padding reads as payload, so the length is
    // pinned by value and not merely by the call existing. It renders in hex: 0x20.
    const output = "ab".repeat(128);
    const bytes = await buildIntent(
      managedDir(),
      respondInput({ circuit: "respondBidirectional", serializedOutput: output, outputLen: 32 }),
    );

    expect(decodeIntent(bytes).actions).toHaveLength(1);
    expect(pushedCallArgs(bytes)).toBe(`pushs <[${output}, 20, ${X}, ${Y}, ${S}, ${ZERO}]: b128b1b32b32b32b1>`);
  });

  it("distinguishes an empty output from a full one, so the length is never assumed", async () => {
    // The zero case is the one a dropped `outputLen` would silently imitate.
    const output = "cd".repeat(128);
    const bytes = await buildIntent(
      managedDir(),
      respondInput({ circuit: "respondBidirectional", serializedOutput: output, outputLen: 0 }),
    );

    expect(pushedCallArgs(bytes)).toBe(`pushs <[${output}, ${ZERO}, ${X}, ${Y}, ${S}, ${ZERO}]: b128b1b32b32b32b1>`);
  });

  it("rejects respondBidirectional without its output", async () => {
    await expect(
      buildIntent(managedDir(), respondInput({ circuit: "respondBidirectional" })),
    ).rejects.toThrow(/serializedOutput/);
  });
