...
[`regenerate.ts`](./regenerate.ts) remains the capture harness for the LIVE tier: when the storage-line stack (contract `@sig-net/* 0.15.0`, release commit `b270880`) is deployed, re-point its genesis/address/height constants at that deploy and capture a singleton state at a block where a respond landed. That capture feeds `chain-midnight`'s ignored live test, not anything in this package.

The read-path captures and goldens that used to live here belong to `chain-signatures/chain-midnight/fixtures/`, which owns its own set.
