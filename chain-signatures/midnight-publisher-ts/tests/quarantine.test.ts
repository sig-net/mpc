...
//
// The funding seed is read in exactly one place, it is never carried on a value
// anything else renders, and it cannot reach a reply line or an error message: this
// process writes its replies to stdout, and stdout is the parent node's stdin, so a
// seed on it would be a seed in that node's log.
