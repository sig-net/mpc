...
use crate::errors::{InitError, JoinError, PublicKeyError, RespondError, SignError, VoteError};
...
impl MpcContract {
    ...
    fn remove_request(&mut self, request: SignatureRequest) -> Result<(), SignError> {
...
impl VersionedMpcContract {
    ...
    pub fn respond(
        ...
        if let ProtocolContractState::Running(_) = protocol_state {
            ...
            let pk = self
                .public_key()
                .map_err(|_| RespondError::ProtocolNotInRunningState)?;
        ...
        match protocol_state {
            ProtocolContractState::Running(RunningContractState {
                ...
                if participants.contains_key(&signer_account_id) {
                    return Err(JoinError::JoinAlreadyParticipant);
                }
                candidates.insert(
    ...
    #[handle_result]
    pub fn vote_join(&mut self, candidate_account_id: AccountId) -> Result<bool, VoteError> {
        log!(
    ...
    #[handle_result]
    pub fn vote_leave(&mut self, kick: AccountId) -> Result<bool, VoteError> {
        log!(
    ...
    #[handle_result]
    pub fn vote_pk(&mut self, public_key: PublicKey) -> Result<bool, VoteError> {
        log!(
    ...
    #[handle_result]
    pub fn vote_reshared(&mut self, epoch: u64) -> Result<bool, VoteError> {
        log!(
    ...
    #[handle_result]
    pub fn vote_update(&mut self, id: UpdateId) -> Result<bool, VoteError> {
...
impl VersionedMpcContract {
    ...
    #[init(ignore_state)]
    #[handle_result]
    pub fn migrate() -> Result<Self, InitError> {
    ...
    ) -> Result<SignatureResponse, SignError> {
        match self {
    ...
    ) -> Result<SignatureResult<SignatureResponse, SignaturePromiseError>, SignError> {
        match self {
            Self::V0(mpc_contract) => {
