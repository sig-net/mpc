...
mod tests {
    ...
    /// A publisher that fails and, like Midnight on an unresolved submit, refuses the retry.
    #[derive(Default)]
    ...
    #[async_trait::async_trait]
    impl ChainPublisher for TerminalPublisher {
        async fn publish_signature(&self, _action: &PublishAction) -> anyhow::Result<()> {
    ...
    async fn publisher_retry_policy_can_make_a_failure_terminal() {
        let publisher = Arc::new(TerminalPublisher::default());
