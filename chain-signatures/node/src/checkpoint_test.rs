...
mod tests {
    ...
    use crate::{
        checkpoint::{CheckpointManager, PendingTxStatus},
        protocol::Chain,
    };
    ...
    async fn test_checkpoint_manager_basic_flow() {
        ...
        checkpoint_manager
            .new_sign_and_respond_request(chain, sign_id, block_height)
            .await;

        let checkpoint = checkpoint_manager.get_chain_checkpoint(&chain).await;
        ...
        assert_eq!(
            pending_tx.unwrap().status,
            PendingTxStatus::PendingSignAndRespond
        );
        ...
        let updated = checkpoint_manager
            .published_signed_transaction(&chain, &sign_id, tx_id, mock_tx)
            .await;
        assert!(updated);
        ...
        assert_eq!(
            pending_tx.unwrap().status,
            PendingTxStatus::PendingExecuteRespond
        );
        ...
        checkpoint_manager
            .published_signed_output(chain, &sign_id)
            .await;
    ...
    async fn test_checkpoint_manager_multiple_chains() {
        ...
        checkpoint_manager
            .new_sign_and_respond_request(eth_chain, sign_id1, 100)
            .await;

        checkpoint_manager
            .new_sign_and_respond_request(sol_chain, sign_id2, 200)
            .await;
    ...
    async fn test_checkpoint_manager_dependencies() {
        ...
        let sign_id = SignId::new([1; 32]);
        let source_chain = Chain::Ethereum;
        let target_chain = Chain::NEAR;

        // Add request to source chain
        checkpoint_manager
            .new_sign_and_respond_request(source_chain, sign_id, 100)
            .await;

        // Add dependency for target chain
        checkpoint_manager
            .new_sign_and_respond_request(target_chain, sign_id, 50)
            .await;
