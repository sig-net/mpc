...
use crate::storage::triple_storage::{TriplePairTaken, TriplePairTakenDropper};
...
pub struct PresignatureGenerator {
    ...
    dropper: TriplePairTakenDropper,
    created: Instant,
    timeout: Duration,
...
/// Abstracts how presignatures are generated by providing a way to request a new presignature that will be
/// complete some time in the future and a way to take an already generated presignature.
pub struct PresignatureSpawner {
    ...
    posits: Posits<FullPresignatureId, TriplePairTaken>,
...
impl PresignatureSpawner {
    ...
    async fn process_posit(
        ...
        let internal_action = if !id.validate() {
            ...
            tracing::warn!(
                ...
                ?from,
                ?action,
                "presignature required triple pair is not known"
            );
            PositInternalAction::Reply(PositAction::Reject)
        } else {
    ...
    /// Propose a new presignature generation protocol to the network.
    async fn propose_posit(&mut self, active: &[Participant]) {
        // To ensure there is no contention between different nodes we are only using triple pairs
        // that we proposed. This way in a non-BFT environment we are guaranteed to never try
        // to use the same triple pair as any other node.
        // TODO: have this part be a separate task such that finding a triple pair is done in parallel instead
        // of waiting for storage to respond here.
        let Some(triple_pair) = self.triples.take_mine(self.me).await else {
            return;
        };

        let pair_id = triple_pair.artifact.id;
        // use holders (not original participants) since some nodes may have lost the artifact.
        let Some(holders) = triple_pair.artifact.holders() else {
        ...
        self.posits.propose(id, triple_pair, &participants);
    ...
    async fn generate(
        &mut self,
        id: FullPresignatureId,
        positor: Positor<TriplePairTaken>,
        participants: &[Participant],
        timeout: Duration,
    ) -> Result<(), InitializationError> {
        let (owner, triple_pair) = match positor {
            Positor::Proposer(proposer, triple_pair) => {
                (proposer, PendingTriplePair::Available(triple_pair))
            }
            Positor::Deliberator(proposer) => (
                proposer,
                PendingTriplePair::InStorage(id.pair_id, self.triples.clone()),
            ),
        };
        tracing::info!(
            ?id,
        ...
        let task = async move {
            let Some(triple_pair) = triple_pair.fetch(owner, timeout).await else {
                return;
            };

            let (pair, dropper) = triple_pair.take();
...
enum PendingTriplePair {
    Available(TriplePairTaken),
    InStorage(TripleId, TripleStorage),
}

impl PendingTriplePair {
    async fn fetch(self, owner: Participant, timeout: Duration) -> Option<TriplePairTaken> {
        let (pair_id, storage) = match self {
            Self::InStorage(pair_id, storage) => (pair_id, storage),
            Self::Available(triple_pair) => return Some(triple_pair),
        };
        ...
        let result = tokio::time::timeout(timeout, async {
            ...
            loop {
                interval.tick().await;
                if let Some(triple_pair) = storage.take(pair_id, owner).await {
                    break triple_pair;
                };
            }
        })
        ...
        match result {
            Ok(triple_pair) => Some(triple_pair),
            Err(_) => {
