//! Signature generation: runs the threshold-signatures signing protocol once a posit round agrees on a presignature and participant set.
...
impl SignGenerator {
    /// Fetch the committed presignature, apply the delta, and build the threshold-signatures signing protocol.
    pub(crate) async fn new(
        ...
        let msg_hash_bytes: [u8; 32] = request.args.payload.to_bytes().into();
        let tweak = Tweak::new(request.args.epsilon);
            ...
            match action {
                ...
                Action::Wait => {
                    ...
                    if let Err(err) = self.protocol.message(msg.from, msg.data) {
                        tracing::warn!(?err, "failed to process message in signature protocol");
                    }
                }
...
fn build_publish_state(
    ...
    output: &threshold_signatures::ecdsa::Signature,
