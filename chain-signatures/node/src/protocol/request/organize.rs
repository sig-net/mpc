...
impl OrganizingPhase {
    ...
    pub async fn advance(&mut self, ctx: &mut SignTask, state: &mut SignState) -> SignPhase {
        ...
        // Checkpoint requests bypass the concurrency limiter to ensure they are processed immediately
        // and not delayed by regular signature request limits.
        if is_proposer && !matches!(state.request.kind, SignKind::Checkpoint(_)) {
