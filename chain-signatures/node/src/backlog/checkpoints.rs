...
mod tests {
    ...
    async fn missing_stored_version_keeps_pending_and_persists() {
        ...
        assert!(!checkpoints
            ...
            .await
            .unwrap());
        assert_eq!(checkpoints.count(pending.chain), 1);
        assert_eq!(
            checkpoints
                .storage()
    ...
    async fn mismatched_parser_version_evicts_pending_keeps_latest() {
        ...
        assert_eq!(checkpoints.count(chain), 0);
        assert_eq!(
            checkpoints.latest(chain).await.unwrap(),
            Some(latest.clone())
        );
        assert_eq!(checkpoints.count(chain), 0);
        assert_eq!(
    ...
    async fn parser_version_mismatch_leaves_other_chain_pending() {
        ...
        assert!(checkpoints
            ...
            .await
            .unwrap());
        assert_eq!(checkpoints.count(Chain::Ethereum), 0);
        assert_eq!(checkpoints.count(Chain::Solana), 1);
        assert_eq!(
            checkpoints
                .storage()
