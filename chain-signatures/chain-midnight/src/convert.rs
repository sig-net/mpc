...
//!
//! The signing payload is built ONLY via [`crate::serialized_transaction`],
...
pub fn to_sign_request(
    ...
    let path = path_render(&record.path)?;
...
/// Decision D2, CLOSED: render `path: Bytes<32>` as NUL-trimmed ASCII,
/// failing closed. Never lossy, never a hex fallback, never content
...
/// `"caller-path"`), pinned here by the A9 epsilon golden.
pub fn path_render(path: &[u8; 32]) -> anyhow::Result<String> {
    render_padded_ascii(path, "path")
}
...
mod tests {
    ...
    use crate::{payload_scalar, serialized_transaction};
    use mpc_chain_integration_core::utils::hashing::hash_payload;
    ...
    fn every_event_field_maps_per_the_table() {
        ...
        assert_eq!(event.sender, READ_ADDRESS, "sender is the READ address");
        assert_eq!(
    ...
    fn path_render_fails_closed() {
        ...
        assert_eq!(
            path_render(&ascii_padded::<32>(b"caller-path")).expect("ascii renders"),
            "caller-path"
        );
        // The oracle's empty-path vector derives with "", so all-NUL is legal.
        assert_eq!(path_render(&[0u8; 32]).expect("empty renders"), "");

        let err = path_render(&[0xff; 32])
            .expect_err("non-UTF-8 path bytes must fail closed")
            .to_string();
        assert!(err.contains("path"), "err: {err}");

        let err = path_render(&ascii_padded::<32>(b"a\0b"))
