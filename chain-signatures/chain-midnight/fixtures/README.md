...
3. For each height, run the ignored `rpc::tests::capture_block_fixtures` test with `MIDNIGHT_NODE_URL`, `MIDNIGHT_CAPTURE_BLOCK`, `MIDNIGHT_CAPTURE_SINGLETON`, and a fresh `MIDNIGHT_CAPTURE_OUT_DIR`. Set `MIDNIGHT_CAPTURE_CALLER` on the notify height so the same run also captures caller state. The tool refuses to replace an existing output and prints the finalized block hash, extrinsic index, status, ledger transaction hash, and ordered `ContractCall` addresses.
