...
*What it takes.* Every holder binds the artifact to its first consumer: on first use it persists presignature\_id → sign\_id and refuses that presignature under any other sign\_id, keeping the record as long as the request can be retried. The binding must reach durable storage *before* any message derived from the artifact is sent, or a holder that crashes and recovers resurrects the share and honestly serves a second request. Two rules make it meaningful: only the artifact's recorded owner initiates consumption, checked by every holder, and every message names its instance, so a node feeds each one only to the instance it names. All three are local; no distributed agreement is involved. An honest holder then refuses a second request whichever quorum asks, so with f \< t corrupted nodes fewer than t shares ever exist for the artifact and no second signature can be formed.

*What holds today.* Holders delete the artifact on first use instead of binding it, so a later attempt fails only at a holder that already used it. That yields the property per node, no honest node serving the same artifact twice, but not per artifact: two **disjoint** sets of honest holders can each serve their own first request on it. Both instances need a participant list of at least t (the posit phase starts an instance only once at least t peers have accepted), and an honest holder can appear in only one, but a corrupt holder can serve both. The two lists must therefore overlap only in corrupt nodes: 2t − f holders in total, i.e. 2(t − f) honest ones alongside the f corrupt. Opening either instance requires being the recorded owner, so the owner must be corrupt too, one node and well inside f \< t.

The honest-holder cost falls as f grows: at f \= t − 1 it is two. Holders are all accepters of the artifact's generation, so a presignature can have up to n of them, and sets that large arise in normal operation, not only under attack. Until holders bind, the property holds only for artifacts with fewer than 2t − f holders.
