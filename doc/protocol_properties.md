...
∆timeout is not a constant: the round budget follows the shared schedule `round_timeout(r)` (a short round 0, then geometric growth from a short floor to a hard ceiling), so rotating past the f \+ p proposers that cannot deliver costs the sum of `round_timeout` over the failed rounds. That sum is dominated by the round-0 budget while that count is small (5 s, then 2 s, 2.3 s, 2.6 s, ...): the ceiling is reached only after tens of rounds, so (f \+ p) times the ceiling is a bound, not an estimate.
...
*Enforcement*: 
The proposer starts as soon as t invitees have accepted (itself included) plus a short slack for extra holders, or immediately if every invitee has answered. A larger invite set cannot hold the round for the slowest Accept; late accepters miss Start and retry next round. Silent invitees do not hold the round hostage either: slack and the round budget are the only waits past t. An alternative proposer gets a chance after ∆timeout.
