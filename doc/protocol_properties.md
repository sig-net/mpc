...
*The stronger rule: bind the artifact to its first consumer.* Instead of merely deleting the artifact on first use, persist presignature\_id → sign\_id and reject that presignature under any other sign\_id, retaining the tombstone at least as long as the request can be retried. Still local, still single-writer, no agreement involved. An honest holder then refuses the second instance whichever quorum asks, so with f \< t corrupted nodes fewer than t shares ever exist for a second request and no second signature can be formed. Without that rule, the property holds only for artifacts with fewer than 2t − f holders.
