...
- Count-keyed maps: walk a rid's entries `0..counter-1` with a per-rid growth watermark, per-entry decode-or-skip — a poisoned entry never blocks a later genuine one. Used by client response polling; live discovery reads entries out of tx transcripts instead (§5.1).
