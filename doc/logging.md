...
| OTLP | OpenTelemetry collector → tracing backends | `MPC_OTLP_ENDPOINT` / `--otlp-endpoint` (default `http://localhost:4318`) | `--opentelemetry-level` (`MPC_OPENTELEMETRY_LEVEL`), default `off` — `off`/`info`/`debug`/`trace` |
| Stackdriver | GCP Cloud Logging, structured stderr | on GCP unless `MPC_DISABLE_GCP_LOGS` / `--disable-gcp-logs` | `RUST_LOG` |

Traces (spans) over OTLP are unaffected by the level knob — they are sampled separately in
`logs.rs`.

### Local OTLP setup

1. Start a local collector, e.g. [Jaeger all-in-one](https://www.jaegertracing.io/docs/getting-started/).
2. Run the node with the level enabled, e.g. `MPC_OPENTELEMETRY_LEVEL=debug` (default `off` exports
   nothing); it exports to `http://localhost:4318` by default; override with `MPC_OTLP_ENDPOINT` or
   `--otlp-endpoint`.
