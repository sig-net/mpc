...
| 2 | Orphan posit mailboxes are unbounded | D5 | unfiled ([#793](https://github.com/sig-net/mpc/issues/793) adjacent) | small |
| 3 | Organizing waits for the active set with no timeout | L1 | unfiled ([#876](https://github.com/sig-net/mpc/issues/876) adjacent) | small, needs a decision |
...
## 2. D5: bound the orphan posit mailboxes
...
## 3. L1 caveat (i): bound the wait for the active set
...
## 4. L4: report sync results per peer
...
## 5. S1 hardening: bind a presignature to its first sign id
...
inside the fsync window (item 6) or by a restored backup is served honestly a
second time.
...
## 6. D3: make "durably" true, or say what it is worth
...
2. Decide on `appendfsync`. `always` makes S1's crash argument sound at a per-write
   ...
   one second window. Item 5 changes this trade: with a binding record written in
   the same script as the delete, a resurrected pair is caught at the second take,
   so `everysec` becomes defensible. Decide 5 and 6 together.
